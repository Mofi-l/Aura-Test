...
// @updateURL    https://raw.githubusercontent.com/Mofi-l/Aura-Test/main/aura.meta.js
// @downloadURL  https://raw.githubusercontent.com/Mofi-l/Aura-Test/main/aura.user.js
...
(function() {
    ...
    const SharedFunctions = {
            ...
            `;
                ...
                togglePassword.addEventListener('click', () => {
                    ...
                    togglePassword.innerHTML = type === 'password' ?
                        '<svg viewBox=ox="0 0 24 24" class="eye-icon"><path d="M12 4.5C7 4.5 2.73 7.61 1 12c1.73 4.39 6 7.5 11 7.5s9.27-3.11 11-7.5c-1.73-4.39-6-7.5-11-7.5zm0 12.5c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5zm0-8c-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3-1.34-3-3-3z"/></svg>' :
        ...
        // 2. Style Injection Function
        async injectAuthStyles() {
        ...
        // 4. SDK Loading Functions
         async loadCognitoSDK() {
            return new Promise((resolve, reject) => {
                ...
                script.onload = () => {
                    debugLog('Amazon Cognito SDK loaded successfully');
                    resolve();
                };
                ...
                script.onerror = () => {
                    ...
                    fallbackScript.onload = () => {
                        debugLog('Fallback Amazon Cognito SDK loaded successfully');
                        resolve();
                    };
        ...
        async loadAwsSdk() {
            return new Promise((resolve, reject) => {
                if (typeof AWS !== 'undefined' && AWS.S3) {
                    debugLog('AWS SDK is already loaded and AWS.S3 is available.');
                    resolve();
                } else {
                    debugLog('Loading AWS SDK...');
                    const script = document.createElement('script');
                    ...
                    script.onerror = (error) => {
                        errorLog('Failed to load AWS SDK', error);
    ...
    `;
        ...
        //6. Username and Avatar
        async  displayUsername() {
            return new Promise((resolve) => {
                debugLog("Attempting to display username...");
                ...
                if (!username) {
                    debugLog("No authenticated username found");
                    return null;
                }

                debugLog("Displaying username:", username);

                const usernameDisplay = createUsernameDisplay();
                if (!usernameDisplay) {
                    errorLog("Failed to create username display element");
                    return null;
                }
                ...
                function tryDisplay() {
                    debugLog(`Attempt ${attempt + 1} to find the widget element...`);
                    ...
                    if (widgetElement) {
                        ...
                        if (username) {
                            ...
                            errorLog("Failed to extract username. 'login' attribute is missing.");
                        ...
                        errorLog("Failed to find widget element after maximum retries.");
                        resolve(null);
                    }
                }
                ...
                function createUsernameDisplay() {
                    ...
                    if (!usernameDisplay) {
                        ...
                        if (widget) {
                            ...
                            errorLog("Widget not found. Cannot display username.");
                        }
                    }
    ...
    const AuthService = {
        ...
        async authenticate(username, password) {
            ...
            if (this.isAuthenticated && this.authToken && !this.isTokenExpired()) {
        ...
        async ensureAuthenticated() {
            ...
            // Request new credentials only if necessary
            const credentials = await SharedFunctions.showAuthModal();
            if (!credentials) {
                throw new Error('Authentication cancelled');
            }

            return await this.authenticate(credentials.username, credentials.password);
        },
    ...
    // Function to check if form should be shown
    function shouldShowProjectForm() {
        const today = new Date().toISOString().split('T')[0];
        const lastFormShow = localStorage.getItem('lastProjectFormShow');
        const formSubmitted = localStorage.getItem('projectFormSubmitted');

        if (lastFormShow === today || formSubmitted === today) {
            return false;
        }
        return true;
    }

    // Checking authentication before loading dependencies
            ...
            try {
                ...
                if (credentials) {
                    ...
                    if (shouldShowProjectForm()) {
                        ...
                        localStorage.setItem('lastProjectFormShow', today);
                    }
                }
            } catch (error) {
    ...
    function clearLocalStorageIfNeeded() {
        ...
        // Only clear if it's after 5 AM and hasn't been cleared today
        if (!lastClearedDate || (lastClearedDate !== todayDate && currentHour >= 5)) {
            ...
            // Clear form-specific items
            localStorage.removeItem('lastProjectFormShow');
            localStorage.removeItem('projectFormSubmitted');

            debugLog('Local storage cleared while preserving authentication.');
        }
    }
    ...
    async function handleProjectDetailsSubmit() {
        try {
            ...
            // Add this line to mark form as submitted
            const today = new Date().toISOString().split('T')[0];
            localStorage.setItem('projectFormSubmitted', today);
    ...
    // Modify your closeProjectDetailsForm function
...
`;
    ...
    async function loadDependencies() {
        try {
            ...
            debugLog('AWS SDK and Cognito SDK loaded successfully');
        } catch (error) {
            errorLog('Failed to load dependencies:', error);
        }
    }

    // Helper functions to save the Authentication credentials
    function saveAuthCredentials(username, password) {
        if (username && password) {
            try {
                localStorage.setItem('lastAuthUsername', username);
                localStorage.setItem('lastAuthPassword', password);
                debugLog('Authentication credentials saved successfully');
                return true;
            } catch (error) {
    ...
    initializeScript().then(() => {
        loadDependencies().then(() => {
            ...
            function setInitialLoadComplete() {
                debugLog('Setting isInitialLoad to false');
                isInitialLoad = false;
                debugLog(`isInitialLoad is now: ${isInitialLoad}`);
            }

            window.addEventListener('load', () => {
                setTimeout(setInitialLoadComplete, 2000);
            });

            setTimeout(setInitialLoadComplete, 5000);
            ...
            async function saveAUXData(entry) {
                ...
                const formattedDate = formatToMMDDYYYY(new Date(entry.date));

                // Wait for username if it's a Promise, otherwise use the stored username
                let username;
                if (entry.username instanceof Promise) {
                    username = await entry.username;
                } else {
                    username = entry.username || localStorage.getItem("currentUsername");
                }

                if (!username) {
                    errorLog("Username is missing. Data cannot be saved.");
                    return;
                }
                ...
                if (lastEntry &&
                    ...
                    debugLog('Skipping duplicate entry within 500ms window');
                    return;
                }
                ...
                if (entry.auxLabel === "Offline - N/A - N/A") {
                    if (!auxData.some(item => item.auxLabel === "Offline - N/A - N/A")) {
                        ...
                        debugLog('Offline entry saved:', newEntry);
                    }
                    ...
                    auxData.push(newEntry);
                    localStorage.setItem('auxData', JSON.stringify(auxData));
                    debugLog('AUX Data saved:', newEntry);
                } else {
                    debugLog('Time spent is less than 5 seconds. Skipping entry.');
                }
            ...
            function saveAllChanges(table, auxData) {
                SharedFunctions.showCustomConfirm('Are you sure you want to save all changes?', (confirmed) => {
                    if (confirmed) {
                        try {
                            ...
                            SharedFunctions.showCustomAlert('All changes saved successfully!');
                            displayAUXData(true);
                        } catch (error) {
                            SharedFunctions.showCustomAlert(error.message);
                        }
                    }
                });
            }
            ...
            function restoreSelections() {
                ...
                if (saved) {
                    try {
                        ...
                        errorLog('Error restoring selections:', error);
                    }
                }
            }
            ///////////////////////////////////////////////////////////////
                ...
                reader.onload = function(e) {
                    ...
                    const restoredAuxData = lines.map(line => {
                        ...
                        if (!timeSpent || timeSpent === "undefined") {
                            errorLog('Invalid timeSpent value:', timeSpent);
                            return null;
                        }
                    ...
                    debugLog('AUX Data merged and restored to localStorage:', combinedAuxData);
                    SharedFunctions.showCustomAlert('AUX Data restored and merged');
                    localStorage.removeItem('manualAUXChange');
                };

                reader.onerror = function() {
                    errorLog('Error reading file:', reader.error);
                };
            ...
            function exportToCSV() {
                try {
                    ...
                    debugLog('AUX Data from localStorage (Parsed):', auxData);
                    ...
                    errorLog('Error exporting AUX data to CSV:', error);
                }
            }
            ///////////////////////////////////////////////////////////////
                ...
                return new Promise((resolve) => {
                    document.getElementById('auth-submit').addEventListener('click', async () => {
                        ...
                        try {
                            ...
                            debugLog('CSV Content:', csvContent);
                            ...
                            errorLog('Error during AWS export:', error);
    ...
    `;
            ...
            async function importFromAws() {
                try {
                    ...
                    try {
                        ...
                        errorLog('Date range selection error:', err);
                        return;
                    }
                    ...
                    try {
                        ...
                        debugLog('Loaded site mapping:', siteMap);

                        for (const prefix of prefixes) {
                            debugLog(`Processing prefix: ${prefix}`);
                            ...
                            debugLog(`Found ${listedObjects.Contents.length} files for prefix ${prefix}`);

                            for (const item of listedObjects.Contents) {
                                debugLog(`Processing file: ${item.Key}`);
                                ...
                                debugLog(`File contains ${rows.length} rows`);

                                // Skip header row
                                for (let i = 1; i < rows.length; i++) {
                                    const row = rows[i].trim();
                                    if (!row) {
                                        debugLog(`Skipping empty row ${i}`);
                                        continue;
                                    }

                                    const columns = row.split(',').map(col => col.trim());
                                    if (columns.length < 4) {
                                        debugLog(`Skipping invalid row ${i}: insufficient columns`);
                                        continue;
                                    }

                                    let processedRow;
                                    if (item.Key.startsWith('Aura_NPT_')) {
                                        debugLog(`Processing Aura NPT row ${i}`);
                                        processedRow = await processAuraNPTRow(columns, username, startDateTime, endDateTime, siteMap);
                                    } else {
                                        debugLog(`Processing aux_data row ${i}`);
                                        processedRow = await processAuxDataRow(columns, username, startDateTime, endDateTime, siteMap);
                                    }

                                    if (processedRow) {
                                        debugLog(`Successfully processed row ${i}:`, processedRow);
                                        relevantData.push(processedRow);
                                    } else {
                                        debugLog(`Row ${i} skipped or failed processing`);
                                    }
                                }
                            }
                        }

                        debugLog(`Total relevant data found: ${relevantData.length} rows`);
                        ...
                        if (relevantData.length > 0) {
                            ...
                            SharedFunctions.showCustomAlert('Data downloaded successfully!');
                        } else {
                            debugLog('No data found. Final debug info:', {
                                dateRange: {
                    ...
                    errorLog('Error in importFromAws:', error);
            ...
            window.addEventListener('error', function(event) {
                if (event.target.tagName === 'SCRIPT') {
                    errorLog('Failed to load script:', event.target.src);
                    SharedFunctions.showCustomAlert('Failed to load required dependencies. Please try again.');
                }
            });
            ...
            async function processAuraNPTRow(columns, username, startDateTime, endDateTime) {
                try {
                    ...
                    if (!rowDate || isNaN(rowDate.getTime())) {
                        debugLog('Invalid date:', columns[0]);
                        return null;
                    }
                    ...
                    errorLog('Error processing Aura NPT row:', error);
                    return null;
                }
            }
            ...
            async function processAuxDataRow(columns, username, startDateTime, endDateTime) {
                try {
                    ...
                    errorLog('Error processing aux_data row:', error);
                    return null;
                }
            }
            ...
            function parseDate(dateString) {
                ...
                errorLog(`Failed to parse date: ${dateString}`);
                return null;
            }
            ...
            function processTimeSpent(timeSpent) {
                ...
                try {
                    ...
                    errorLog('Error processing time:', error);
                    return '00:00:00';
                }
            }
            ...
            function convertToHours(timeString) {
                ...
                try {
                    ...
                    errorLog('Error converting time to hours:', error);
                    return '0';
                }
            }
    ...
    `;
            ...
            async function loadMSSitesData() {
                ...
                try {
                    ...
                    errorLog('Error loading MS_Sites data:', error);
                    return {};
                }
            }
            ...
            async function getFullData() {
                ...
                try {
                    ...
                    try {
                        ...
                        for (const prefix of prefixes) {
                            ...
                            for (const item of listedObjects.Contents) {
                                ...
                                if (item.Key.startsWith('Aura_NPT_')) {
                                    // For Aura_NPT_ files, add as is
                                    if (allData.length === 0) {
                                        allData.push(rows[0]); // Add header only for first file
                                    }
                                    allData.push(...rows.slice(1));
                                } else if (item.Key.startsWith('aux_data_')) {
                                    // For aux_data_ files, transform the data
                                    if (allData.length === 0) {
                                        // If this is the first file, add Aura_NPT_ header
                                        allData.push(AURA_NPT_COLUMNS.join(','));
                                    }

                                    // Transform and add each row
                                    for (let i = 1; i < rows.length; i++) {
                                        if (rows[i].trim()) {
                    ...
                    errorLog('Error in getFullData:', error);
                    SharedFunctions.showCustomAlert('Failed to retrieve full data: ' + error.message);
                ...
                try {
                    ...
                    if (storedUsername && storedPassword) {
                        try {
                            ...
                            debugLog('Stored credentials invalid, requesting new authentication');
                            localStorage.removeItem('lastAuthUsername');
                            localStorage.removeItem('lastAuthPassword');
                        }
                    }
        ...
        `;
                                ...
                                try {
                                    ...
                                    for (const item of listedObjects.Contents) {
                                        try {
                                            ...
                                            errorLog(`Error processing file ${item.Key}:`, fileError);
                                            continue;
                                        }
                                    }
                                } catch (prefixError) {
                                    errorLog(`Error processing prefix ${prefix}:`, prefixError);
                                    continue;
                                }
                            }
            ...
            document.addEventListener("DOMContentLoaded", () => {
                debugLog("DOM fully loaded. Starting to display username...");
                SharedFunctions.displayUsername();
            });
            ///////////////////////////////////////////////////////////////
            //CL Data Manager
    ...
    `;
                ...
                searchBtn.addEventListener('click', () => {
                    ...
                    if (projectNumber) {
                        ...
                        SharedFunctions.showCustomAlert('Please enter a project number');
                    }
                });
    ...
    <span class="loading-text">Searching project time data...</span>
        ...
        `;
            ...
            `;
                    ...
                    errorLog('Error searching project time:', error);
                    resultsContainer.innerHTML = `
            <div class="no-results">
...
:root {
    ...
    `;
        ...
        `;
                    ...
                    errorLog('Error showing cohort dashboard:', error);
                    SharedFunctions.showCustomAlert('Error loading dashboard: ' + error.message);
                } finally {
                    const loadingIndicator = document.querySelector('.loading-indicator');
                    if (loadingIndicator) {
                        loadingIndicator.remove();
...
/* Optional: Add a subtle glow effect to the widget */
#aux-widget:hover {
...
#aux-widget::-webkit-scrollbar {
    width: 8px;
}

#aux-widget::-webkit-scrollbar-track {
    background: rgba(255, 255, 255, 0.05);
    border-radius: 4px;
}

#aux-widget::-webkit-scrollbar-thumb {
    background: rgba(255, 255, 255, 0.1);
    border-radius: 4px;
}

#aux-widget::-webkit-scrollbar-thumb:hover {
    background: rgba(255, 255, 255, 0.15);
}
`;
...
`;
            ...
            // Add this after the toggle button in your existing code
...
`;
            ...
            toggleButton.parentNode.insertBefore(pauseButton, toggleButton.nextSibling);
...
`;
            ...
            makeWidgetDraggable();
            ///////////////////////////////////////////////////////////////
            // Main Flash Data function
            async function flashData() {
                debugLog('Starting flashData function...');
                try {
                    // Authenticate and get AWS credentials
                    const credentials = await SharedFunctions.showAuthModal();
                    if (!credentials) {
                        errorLog('Authentication cancelled by user');
                        throw new Error('Authentication cancelled');
                    }

                    const token = await AuthService.ensureAuthenticated();
                    if (!token) {
                        errorLog('Authentication failed - no token received');
                        throw new Error('Authentication failed');
                    }
                    ...
                    errorLog('Error in flashData:', error);
    ...
    `);
            ...
            function addFunctionsToWindow(newWindow, combinedData) {
                ...
                newWindow.showSiteData = function(site) {
                    ...
                    try {
                        ...
                        errorLog('Error processing site data:', error);
                ...
                newWindow.convertTimeToHours = function(timeString) {
                    ...
                    try {
                        ...
                        errorLog('Error converting time:', error);
                        return 0;
                    }
                };
            ...
            // Add this to initialize the feature
            ...
            // Part 1: Core Dashboard Setup and Initialization
            ...
            document.getElementById('dashboardButton').addEventListener('click', function() {
                SharedFunctions.showCustomAlert('This option is not available for your site')
            });
            ...
            async function showDashboard() {
                try {
                    ...
                    errorLog('Dashboard initialization error:', error);
            ...
            async function startDashboardUpdates() {
                try {
                    ...
                    errorLog('Failed to start dashboard updates:', error);
            ...
            function calculateUserStatus(lastUpdateTime, auxLabel, isPaused) {
                debugLog('Calculating status:', { lastUpdateTime, auxLabel, isPaused }); // Add this log
                if (isPaused) {
                    return 'paused';
                }
            ...
            // Part 2: Dashboard UI Initialization and Styling
...
`;
    ...
    `;
            ...
            // Part 3: Data Fetching and Dashboard Updates
            async function sendAuxUpdate() {
                try {
                    const currentState = JSON.parse(localStorage.getItem('auxState'));
                    debugLog('Attempting to send AUX update:', currentState);
                    if (!currentState) {
                        debugLog('No current AUX state to send');
                        return;
                    }
                    ...
                    if (!username || !password) {
                        debugLog('No stored credentials, requesting authentication...');
                        const credentials = await SharedFunctions.showAuthModal();
                        if (!credentials) {
                            throw new Error('Authentication cancelled');
                        }
                    ...
                    debugLog('Authenticating with AWS...');
                    const token = await AuthService.ensureAuthenticated();
                    ...
                    await new Promise((resolve, reject) => {
                        AWS.config.credentials.get(err => {
                            if (err) {
                                errorLog('Error getting AWS credentials:', err);
                                reject(err);
                            } else {
                                debugLog('AWS credentials successfully obtained');
                                resolve();
                            }
                        });
                    });
                    ...
                    debugLog('Sending data to S3:', data);
                    ...
                    debugLog('Successfully sent data to S3:', data);
                    ...
                    // Trigger immediate dashboard update if dashboard is open
                    if (document.getElementById('aux-dashboard')) {
            ...
            // Add event listener for AUX state updates
            ...
            function calculatePauseDuration(state) {
                if (!state.pauseTime) return 0;
                return new Date().getTime() - new Date(state.pauseTime).getTime();
            }

            // Helper function to broadcast state changes
            function broadcastStateChange(data) {
                const event = new CustomEvent('auxStateChange', {
                    detail: {
                        ...data,
            ...
            async function updateDashboardData(isImmediateUpdate = false) {
                try {
                    debugLog('Updating dashboard data...', isImmediateUpdate ? '(Immediate update)' : '');
                    ...
                    try {
                        ...
                        debugLog('Credential refresh failed, requesting new authentication');
                        const credentials = await SharedFunctions.showAuthModal();
                    ...
                    debugLog(`Found ${response.Contents.length} AUX updates`);
                    ...
                    const updates = await Promise.all(response.Contents.map(async (item) => {
                        try {
                            ...
                            const updateDate = new Date(lastUpdateTime).setHours(0, 0, 0, 0);
                            const timeSinceUpdate = currentTime - lastUpdateTime;

                            // Ensure we're using the stored login time
                            const storedLoginTime = userState.loginTime;
                            const today = new Date().toISOString().split('T')[0];

                            // Only use stored login time if it's from today
                            ...
                            errorLog(`Error processing file ${item.Key}:`, error);
                            return null;
                        }
                    }));
                    ...
                    debugLog('Processed updates:', validUpdates.length);
                    ...
                    // Start the continuous timer updates
                    if (document.getElementById('aux-dashboard')) {
                        if (dashboardAnimationFrame) {
                            cancelAnimationFrame(dashboardAnimationFrame);
                        }
                    ...
                    errorLog('Error updating dashboard:', error);
            ...
            function handleUpdateError(error) {
                if (error.code === 'CredentialsError' ||
                    ...
                    debugLog('Credentials error, requesting re-authentication...');
                    localStorage.removeItem('lastAuthUsername');
                    localStorage.removeItem('lastAuthPassword');
                    stopDashboardUpdates();
                    ...
                    setTimeout(async () => {
                        try {
                            ...
                            errorLog('Re-authentication failed:', authError);
            ...
            window.addEventListener('online', () => {
                debugLog('Connection restored, restarting dashboard updates...');
                if (document.getElementById('aux-dashboard')) {
                    startDashboardUpdates();
                }
            });
            ...
            window.addEventListener('offline', () => {
                debugLog('Connection lost, stopping dashboard updates...');
                stopDashboardUpdates();
                if (document.getElementById('aux-dashboard')) {
            ...
            async function sendAuxUpdateWithRetry(maxRetries = 3, baseDelay = 1000) {
                ...
                while (attempt < maxRetries) {
                    try {
                        await sendAuxUpdate();
                        debugLog('AUX update sent successfully on attempt', attempt + 1);
                        return;
                    } catch (error) {
                        attempt++;
                        if (attempt === maxRetries) throw error;
                        const delay = baseDelay * Math.pow(2, attempt);
                        debugLog(`Retry attempt ${attempt} failed, waiting ${delay}ms before next attempt`);
            ...
            function updateDashboardUI(updates) {
                ...
                if (!tbody) {
                    errorLog('Dashboard table body not found');
                    return;
                }
...
Total Pause Duration: ${formatTime(totalPause)}
...
${update.isPaused ? `Paused Since: ${new Date(update.pauseTime).toLocaleString()}` : ''}`;
                ...
                debugLog('Dashboard UI updated with', updates.length, 'entries');
                debugLog('Current timers:', window.dashboardTimers.length);
            }

            // Helper function to show detailed view
            function showDetailedView(update) {
                SharedFunctions.showCustomAlert(`
    ...
    `;
            ...
            const debouncedHandleAuxStateChange = debounce(handleAuxStateChange, 300);
            ...
            function updateSelections(l1, l2, l3) {
                ...
                debugLog("Updating selections to:", l1, l2, l3);
                ...
                if (l1) {
                    debugLog('Updating L1 selection:', l1);
                    debouncedHandleAuxStateChange(l1Names[l1] + ' - N/A - N/A');
                }
            }
            ///////////////////////////////////////////////////////////////
            //Export functions//
            ...
            window.handleL1Change = async function(event) {
                debugLog(`L1 change event fired. isInitialLoad: ${isInitialLoad}`);
                const l1Value = event.target.value;
                debugLog('New L1 value:', l1Value);
                ...
                if (l2Mapping[l1Value]) {
                    ...
                    if (l1Value !== '' && isTabFocused && !isRestoring) {
                        showCombinedPopup(newAuxLabel, (auditData) => {
                            debugLog('Audit data submitted:', auditData);
                        });
                    }
                }
            ...
            window.handleL2Change = async function(event) {
                debugLog('L2 change event fired');
                const l2Value = event.target.value;
                debugLog('New L2 value:', l2Value);
                const l3Container = document.getElementById('aux-l3-container');
                const l1Value = document.getElementById('aux-l1').value;
            ...
            window.handleL3Change = async function(event) {
                debugLog('L3 change event fired');
                const l3Value = event.target.value;
                debugLog('New L3 value:', l3Value);
                const l2Value = document.getElementById('aux-l2').value;
                const l1Value = document.getElementById('aux-l1').value;
            ...
            async function showCombinedPopup(auxLabel, callback) {
                ...
                debugLog('showCombinedPopup function called');
    ...
    `;
                ...
                document.getElementById('cancel-btn').addEventListener('click', function() {
                    if (wasPreviousConductProject && !currentLabel.includes('conduct project')) {
            ...
            function validateInputs() {
                ...
                if (!projectTitle || !comment || !relatedAudits) {
                    errorLog('One or more required input elements not found');
                    return false;
                }
            ...
            async function showProjectDashboard() {
                try {
                    ...
                    try {
                        ...
                        debugLog(`Found ${listedObjects.Contents.length} project detail files`);
...
`;
                    ...
                    errorLog('Error in showProjectDashboard:', error);
    ...
    `;
            ...
            async function deleteProjectDetails(projectId) {
                try {
                    ...
                    errorLog('Error deleting project details:', error);
            ...
            async function updateProjectDetails(projectId) {
                try {
                    ...
                    try {
                        ...
                        debugLog('No existing file to delete or error deleting:', error);
                    }
                    ...
                    errorLog('Error updating project details:', error);
            ...
            // Modify the togglePause function to handle cross-tab synchronization
                ...
                try {
                    ...
                    if (isPaused) {
                        ...
                        if (auxState.pauseTime) {
                            ...
                            auxState.totalPauseDuration = (auxState.totalPauseDuration || 0) + Math.max(0, pauseDuration);
                        }
                        delete auxState.pauseTime;
                        auxState.isPaused = false;
                    ...
                    if (pauseButton) {
                        ...
                        pauseButton.style.backgroundColor = isPaused ? '#FFA500' : '#4CAF50';
                    }
                    ...
                    errorLog('Error in togglePause:', error);
            ...
            window.addEventListener('auxStateChange', async (event) => {
                ...
                // Update any UI elements that depend on the pause state
                updateUIForPauseState(isPaused, auxState);

                // Force dashboard update if open
                if (document.getElementById('aux-dashboard')) {
                    await updateDashboardData(true);
                }
            });
            ...
            function unifiedTimerUpdate(startTime, auxLabel) {
                const currentTime = Date.now();
                const auxState = JSON.parse(localStorage.getItem('auxState')) || {};

                if (auxState.isPaused) {
                    return auxState.pauseTime - startTime - (auxState.totalPauseDuration || 0);
                }

                const elapsedTime = currentTime - startTime - (auxState.totalPauseDuration || 0);

                // Update widget timer
                const widgetTimer = document.getElementById('aux-timer');
                if (widgetTimer) {
                    const cleanedAuxLabel = auxLabel.replace(/\s*-\s*N\/A\s*/g, '').trim();
                    const auxParts = cleanedAuxLabel.split(' - ').filter(part => part.trim() !== '');
                    const displayedAuxLabel = auxParts[auxParts.length - 1] || 'No AUX available';
                    widgetTimer.textContent = `${displayedAuxLabel} : ${formatTime(elapsedTime)}`;
                }

                // Update dashboard timer if dashboard is open
                const dashboardTimer = document.querySelector('.dashboard-timer');
                if (dashboardTimer) {
                    dashboardTimer.textContent = formatTime(elapsedTime);
                }
            ...
            function updateUIForPauseState(isPaused, auxState) {
                ...
                if (timerElement) {
                    ...
                    if (isPaused) {
                        if (!timerElement.textContent.includes('(Paused)')) {
                            timerElement.textContent += ' (Paused)';
                        }
                    } else {
                        timerElement.textContent = timerElement.textContent.replace(' (Paused)', '');
                ...
                const statusIndicator = document.querySelector('.pause-status-indicator');
                if (statusIndicator) {
                    statusIndicator.style.backgroundColor = isPaused ? '#FFA500' : '#4CAF50';
                    statusIndicator.style.boxShadow = `0 0 5px ${isPaused ? '#FFA500' : '#4CAF50'}`;
                }
            }
            ...
            function updateTimer(startTime, auxLabel, timerElement) {
                if (!startTime || !timerElement) {
                    errorLog('Missing required parameters for updateTimer');
                    return 0;
                }
                ...
                try {
                    ...
                    if (auxState.isPaused) {
                        elapsedTime = Math.max(0, auxState.pauseTime - startTime - (auxState.totalPauseDuration || 0));
                    } else {
                        elapsedTime = Math.max(0, currentTime - startTime - (auxState.totalPauseDuration || 0));
                    }
                    ...
                    errorLog('Timer update error:', error);
                    if (animationFrameId) {
                        cancelAnimationFrame(animationFrameId);
                        animationFrameId = null;
                    }
                ...
                dashboardRows.forEach(row => {
                    ...
                    if (timerCell) {
                        ...
                        if (auxState && auxState.username === username) {
                            const elapsedTime = unifiedTimerUpdate(auxState.startTime, auxState.auxLabel);
            ...
            function startAUXTimer(auxLabel, elapsedTime = 0) {
                ...
                debugLog('AUX state updated in localStorage:', auxLabel);
            ...
            function updateTimerDisplay(auxLabel, elapsedTime) {
                const timerElement = displayTimer();
                if (!timerElement) {
                    errorLog('Timer element not found.');
                    return 0;
                }
                ...
                timerElement.style.opacity = auxState.isPaused ? '0.5' : '1';
                timerElement.style.backgroundColor = auxState.isPaused ? 'rgba(255, 165, 0, 0.1)' : 'rgba(255, 255, 255, 0.8)';
                timerElement.style.borderLeft = auxState.isPaused ? '3px solid #FFA500' : 'none';
                timerElement.style.padding = '10px';
                timerElement.style.marginTop = '10px';
                timerElement.style.borderRadius = '5px';
                ...
                // Update status indicator
                let statusIndicator = timerElement.querySelector('.timer-status-indicator');
                if (!statusIndicator) {
                ...
                statusIndicator.style.backgroundColor = auxState.isPaused ? '#FFA500' : '#4CAF50';
                statusIndicator.style.boxShadow = auxState.isPaused ? '0 0 5px #FFA500' : '0 0 5px #4CAF50';
...
${auxState.startTime ? `Started: ${new Date(auxState.startTime).toLocaleTimeString()}` : ''}`;
                ...
                timerElement.onclick = () => {
                    showTimerDetails(auxState, displayedAuxLabel, actualElapsedTime);
                };
            ...
            // Helper function to show timer details
            ...
            window.addEventListener('timerUpdate', (event) => {
                const { auxLabel, elapsedTime, isPaused } = event.detail;

                // Update any additional UI elements that depend on timer state
                const pauseButton = document.getElementById('pause-button');
                if (pauseButton) {
                    pauseButton.textContent = isPaused ? 'Resume' : 'Pause';
                }
            ...
            async function handleAuxStateChange(newAuxLabel) {
                debugLog(`handleAuxStateChange called with: ${newAuxLabel}`);
                debugLog(`isInitialLoad: ${isInitialLoad}`);
                debugLog(`Current time: ${new Date().toISOString()}`);

                if (isInitialLoad) {
                    debugLog('Skipping AUX state change during initial load');
                    console.trace();
                    return;
                }
                ...
                if (prevState && prevState.auxLabel !== newAuxLabel) {
                    const timeSpent = prevState.isPaused ?
                          calculateTimeSpent(prevState.startTime, prevState.pauseTime) :
                    calculateTimeSpent(prevState.startTime);
                ...
                if (!prevState || prevState.auxLabel !== newAuxLabel) {
                    ...
                    // Create new state with preserved pause duration
                    const newState = {
                        auxLabel: newAuxLabel,
                        startTime: now,
                        timestamp: now,
                        isPaused: false,
                        totalPauseDuration: currentTotalPauseDuration // Preserve the total pause duration
                    };
            ...
            document.addEventListener('DOMContentLoaded', async () => {
                ...
                debugLog('Initial AUX state:', initialAuxState);
                SharedFunctions.injectAuthStyles();
                cleanupTimer();
                clearDailyTimes();
                setInitialLoginTime();
                ...
                isInitialLoad = false;
                debugLog('Initial load complete, isInitialLoad set to false');
            });
            ...
            versionText.addEventListener('mouseenter', () => {
                fetch(gistURL)
                    ...
                    .catch(err => {
                    errorLog('Failed to fetch quotes:', err);
                    versionText.setAttribute('title', 'Error fetching quote.');
                });
            });

        }); // Close loadDependencies().then()
    }); // Close initializeScript().then()

})(); // Close and invoke the main IIFE
